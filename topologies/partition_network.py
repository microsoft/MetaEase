...
os.makedirs("./partition_log/", exist_ok=True)
    ...
    for num_partitions in num_partitions_list:
        for topo_name, is_topo_zoo in topo_name_list:
            ...
            for pid in np.unique(partition_vector):
                node_indices = np.argwhere(partition_vector == pid).flatten().tolist()
                nodes = [node_list[i] for i in node_indices]
                ...
                for num_shortest_paths in num_shortest_paths_list:
                    for (node1, node2) in itertools.combinations(subgraph_g.nodes(), 2):
                        ...
                        # print(paths)
                        for s_path in paths:
                            path_indices = [node_list.index(node) for node in s_path]
                            partitions = np.unique(partition_vector[path_indices])
                            if len(partitions) > 1:
                                # print(partitions)
            ...
            with open(log_path, "w") as fp:
                ...
                for sp in num_shortest_paths_list:
                    fp.writelines(f'{sp}-shortest paths num intra cluster paths {num_intra_cluster_paths_dict[sp]}, num total paths {num_total_paths_dict[sp]} frac {num_intra_cluster_paths_dict[sp] / (num_total_paths_dict[sp] + 1e-6)}\n')
