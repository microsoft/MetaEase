...
class FMPartitioning(AbstractPartitioningMethod):
    ...
    def _partition_impl(self, G, topo_name = "", all_tm_files=[]):
        topo = topo_name
        # write weighted pairs to file
        rnd_number = topo_name + str(np.random.random() * 1000)
        wpfile = os.path.join(self.run_folder,  rnd_number + ".wpairs")
        ...
        currgroup = -1
        with open(os.path.join(self.run_folder, rnd_number + "-fc_a.groups"), "r") as groups:
            for line in groups:
