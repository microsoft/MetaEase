...
# Allow custom data directory to be specified via environment variable
if "DOTE_CUSTOM_DATA_DIR" in os.environ:
    # Use custom data directory
    # print(f"Using custom data directory: {os.environ['DOTE_CUSTOM_DATA_DIR']}")
    custom_data_dir = os.environ["DOTE_CUSTOM_DATA_DIR"]
    # Set up paths for custom data directory - use the original DOTE installation
    original_dote_dir = os.path.dirname(os.path.abspath(__file__))
    sys.path.append(os.path.join(original_dote_dir, "networking_envs"))
    sys.path.append(os.path.join(original_dote_dir, "openai_baselines"))

    # Override the current working directory to make DOTE think it's running from the networking_envs directory
    # This fixes the path construction in env_args_parse.py
    os.chdir(os.path.join(original_dote_dir, "networking_envs"))
else:
    # Original behavior - require networking_envs in current directory
...
class DmDataset(Dataset):
    def __init__(self, props=None, env=None, is_test=None):
        ...
        if props.hist_len > 0:
            ...
            assert props.hist_len == 0
            for histid in range(len(tms)):
                start_idx = histid * num_nodes * (num_nodes - 1)
                end_idx = (histid + 1) * num_nodes * (num_nodes - 1)
                X_.append(np_tms_flat[start_idx:end_idx])
                assert np.all(X_[-1] == tms[histid][0])
...
# Override graph path if custom data directory is specified
if "DOTE_CUSTOM_DATA_DIR" in os.environ:
    custom_data_dir = os.environ["DOTE_CUSTOM_DATA_DIR"]
    # Override the graph path to use our custom data directory
    graph_name = props.ecmp_topo + "/" + props.ecmp_topo + '_int.pickle.nnet'
    props.graph_path = os.path.join(custom_data_dir, graph_name)
    # Also override the base paths to use custom data directory
    props.graph_base_path = custom_data_dir + "/"
    props.hist_location = custom_data_dir + "/"
...
if props.so_mode == SOMode.TRAIN: #train
    ...
    if props.hist_len > 0:
        ...
        assert props.hist_len == 0
        model = NeuralNetwork(env.get_num_nodes()*(env.get_num_nodes()-1), env._optimizer._num_paths)
    model.double()
    # optimizer
    ...
    #load the model (PyTorch 2.6+: weights_only defaults to True; allow loading full object)
    model = torch.load('model_dote.pkl', weights_only=False)
    model.eval()
    with torch.no_grad():
        with tqdm(test_dl) as tests:
            ...
            for (inputs, targets) in tests:
                ...
                # compute served flow value for MAXFLOW
                if props.opt_function == "MAXFLOW":
                    num_nodes = env.get_num_nodes()
                    y_true = torch.narrow(targets, 1, 0, num_nodes * (num_nodes - 1))
                    opt = targets[0][num_nodes * (num_nodes - 1)].item()
                    y_pred = pred + 0.1
                    edges_weight = paths_to_edges.transpose(0,1).matmul(torch.transpose(y_pred, 0, 1))
                    alpha = torch.max(edges_weight.divide(torch.tensor(np.array([env._capacities])).transpose(0,1)))
                    max_flow_on_tunnel = y_pred / alpha
                    ...
                    max_mcf = torch.sum(torch.minimum(max_flow_per_commodity.transpose(0,1), y_true))
                    # loss_val = 1.0 if opt == 0.0 else max_mcf.item()/SizeConsts.BPS_TO_GBPS(opt)
                    # Use the same computation as in the loss function for consistency
                    # This gives us the raw flow value in the same units as the loss function
                    total_served_flow += float(max_mcf.item()) #* 1000000000
            avg_loss = sum(test_losses) / len(test_losses)
            print(f"Test Error: \n Avg loss: {avg_loss:>8f} \n")
            if props.opt_function == "MAXFLOW":
                print(f"Served flow value: {total_served_flow}")
            #print statistics to file
            # Use custom data directory if specified, otherwise use original path
            if "DOTE_CUSTOM_DATA_DIR" in os.environ:
                stats_dir = os.path.join(os.environ["DOTE_CUSTOM_DATA_DIR"], props.ecmp_topo)
            else:
                stats_dir = props.graph_base_path + '/' + props.ecmp_topo
            os.makedirs(stats_dir, exist_ok=True)
            with open(os.path.join(stats_dir, 'so_stats.txt'), 'w') as f:
                import statistics
            ...
            if concurrent_flow_cdf != None:
                ...
                with open(os.path.join(stats_dir, 'concurrent_flow_cdf.txt'), 'w') as f:
