"""
MetaEase Main Optimization Pipeline.

This module implements the core MetaEase algorithm that combines:
1. Symbolic Execution (KLEE): Generates path-representative seed inputs by exploring
   distinct code paths in the heuristic implementation
2. Gradient Ascent: Maximizes the performance gap using gradients computed via:
   - Benchmark gradient: From Lagrangian duality (no re-solving needed)
   - Heuristic gradient: From Gaussian Process surrogate fitted to heuristic evaluations
3. Path-Aware Updates: Ensures gradient steps stay within the same code path

The algorithm works in iterations:
- Each iteration optimizes a partition of input variables (for scalability)
- KLEE generates seeds for the current partition
- Gradient ascent refines each seed to maximize gap
- Best results are carried forward to next iteration
"""
...
def main(
    ...
    """
    Main MetaEase optimization pipeline.

    Args:
        problem_description: Dictionary containing problem configuration
            - problem_type: Type of problem (TE, knapsack, vbp, etc.)
            - topology: Network topology name (for TE problems)
            - Other problem-specific parameters
        base_save_dir: Base directory for saving results
        klee_task: KLEE execution mode
            - "inputs_scale_fixed_points": Partition variables, optimize one partition at a time
            - "inputs": Optimize all variables together

    Returns:
        None (results are saved to files in save_dir)

    Algorithm Overview:
        1. Partition input variables into clusters (for scalability)
        2. For each cluster:
           a. Fix variables not in current cluster to best values from previous iterations
           b. Run KLEE to generate path-representative seeds for current cluster
           c. For each seed, run gradient ascent to maximize gap
    ...
    # Initialize problem instance (contains heuristic implementation and benchmark solver)
    problem = get_problem_instance(problem_type, config_path)
    all_klee_vars = problem.all_klee_var_names  # Variables that KLEE will make symbolic
    logger.debug(f"All klee vars: {all_klee_vars}")

    # Track best solution found so far
    max_global_gap = float("-inf")  # Maximum gap = Benchmark - Heuristic
    best_global_sample = None  # Input values that achieved max_global_gap
    best_global_optimal_all_vars = None  # Optimal solution variables
    best_global_heuristic_all_vars = None  # Heuristic solution variables
    already_optimized_vars = (
        set()
    )  # Track which variables have been optimized (for fixed-point strategy)
    start_time = time.time()

    # Partition variables into clusters for scalable optimization
    # This allows us to optimize subsets of variables while fixing others
    if klee_task == "inputs_scale_fixed_points":
        # Option 1: Use pre-computed clusters from MetaOpt (if available)
        if parameters["use_MetaOpt_cluster"]:
            variable_names_clusters = get_clusters_from_directory(
                problem_type, all_klee_vars, parameters["cluster_path"]
            )
        else:
            # Option 2: Generate clusters automatically (e.g., based on graph structure)
            # Each cluster contains at most max_num_scalable_klee_inputs variables
        ...
        # Optimize all variables together (no partitioning)
    ...
    klee_iterations = len(
        variable_names_clusters
    )  # Number of KLEE iterations (one per cluster)
    optimization_start_time = time.time()

    # Track the best input that led to the gap (for fixed-point initialization)
    best_gap_input = None

    # Main optimization loop: process each cluster of variables
    for klee_idx, cluster_variables in enumerate(variable_names_clusters):
        ...
        # Track best solution for this cluster
        cluster_max_gap = float("-inf")
        cluster_best_sample = None
        cluster_best_optimal_all_vars = None
        cluster_best_heuristic_all_vars = None

        # Partition strategy: fix variables not in current cluster
        # This allows us to optimize one partition at a time for scalability
        all_variables = set(all_klee_vars)
        fixed_variables = all_variables - set(
            cluster_variables
        )  # Variables to keep fixed

        # Load best sample from previous iteration to initialize fixed variables
        # This warm-start strategy improves convergence
        prev_best_sample_path = os.path.join(
            save_dir, f"best_global_sample_kleeIteration{klee_idx - 1}.json"
        )

        # Determine which variables to fix and their values
        # Fixed variables use best values from previous iterations
        ...
        # Run multiple rounds of KLEE until we find a non-zero gap
        # This handles cases where KLEE initially finds only zero-gap inputs
        for non_zero_round in range(parameters["num_non_zero_rounds"]):
            ...
            # Early stopping: if we found a better gap, we can stop this round
            if (
            ...
            # Path to save KLEE-generated seed inputs for this round
            ...
            if (
                not os.path.exists(klee_path)
                and not parameters["disable_klee"]
                and not parameters.get("seed_file", False)
            ):
                # run klee program
                start_time = time.time()
                # Configure KLEE timeout based on parameters or use a reasonable default
                klee_timeout = parameters.get(
                    "max_time_per_klee_point", 1800
                )  # Default 30 minutes
                ...
                if non_zero_round != 0:
                    ...
                    for exclusion_path in all_exclusion_paths:
                        if os.path.exists(exclusion_path):
                            ...
                            logger.info(
                                f"Exclusion path {exclusion_path} does not exist"
                            )
            ...
            if parameters.get("seed_file", False):
                ...
                logger.info(
                    f"Loaded {len(klee_samples)} samples from seed file {parameters['seed_file']}"
                )
            ...
            if os.path.exists(klee_path):
                ...
                if len(klee_samples) == 0:
                    ...
                    KLEE_ONLY_ONE_CODE_PATH_OR_FAILED = True
                    logger.info(
                        f"KLEE path {klee_path} has 1 sample, one code path is only available"
                    )
            ...
            if not parameters["disable_klee"] and not parameters.get(
                "seed_file", False
            ):
            ...
            if not parameters.get("seed_file", False) and (
                ...
                or problem.problem_config["problem_type"]
                in ["arrow", "PoP", "knapsack"]
            ):
                thresholds = problem.get_thresholds(
                    {var: 0.0 for var in problem.all_klee_var_names}
                )
                logger.info(
                    f"Adding Random samples because disable_klee is True or klee_path has one (one code-path) or 0 samples (failed klee)"
                )
                num_random_seed_samples = parameters["num_random_seed_samples"]
                vars = problem.all_klee_var_names
                random_samples = {
                    f"random_sample_{i}": {
                        var: random.randint(thresholds[var][0], thresholds[var][1])
                        for var in vars
                    }
            ...
            try:
                ...
                logger.error(
                    f"NOTICE: Setting the round_best_sample to the existing best_global_sample"
                )
                ...
                round_best_optimal_all_vars = (
                    best_global_optimal_all_vars.copy()
                    if best_global_optimal_all_vars is not None
                    else None
                )
                round_best_heuristic_all_vars = (
                    best_global_heuristic_all_vars.copy()
                    if best_global_heuristic_all_vars is not None
                    else None
                )
            ...
            if round_max_gap >= cluster_max_gap:
                logger.info(
                    f"Updating cluster best - round_max_gap: {round_max_gap}, cluster_max_gap: {cluster_max_gap}"
                )
    ...
    gap, gradient, (optimal_value, heuristic_value), all_vars, code_path_num = (
        problem.get_gaps_process_input(best_global_sample)
    )
    logger.info(f"Optimal values: {optimal_value}")
    logger.info(f"Heuristic values: {heuristic_value}")
    logger.info(
        f"Number of optimal value calls: {problem.num_compute_optimal_value_called}"
    )
    logger.info(
        f"Number of heuristic value calls: {problem.num_compute_heuristic_value_called}"
    )
...
def metaease_main(args):
    ...
    Args:
        args: Command-line arguments containing:
            - problem: Problem specification (e.g., "TE_DemandPinning_abilene")
            - base_save_dir: Directory to save results
            - klee_task: KLEE execution mode ("inputs_scale_fixed_points" or "inputs")

    The method follows this pipeline:
    1. Parse problem description from command-line arguments
    2. Run the main optimization loop (partitioning, KLEE, gradient ascent)
    3. Save results to the specified directory
    """
